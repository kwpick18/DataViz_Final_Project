...
ui <- fluidPage(
  ...
  tabPanel("IceTime", fluid = TRUE, 
           ...
           sidebarLayout(sidebarPanel(
            ...
            label = "Choose a Season", choices= levels(factor(powerplay_2017_all$season)), selected = "2008")), 
...
server <- function(input, output, session) {
  ...
  iceTime_sub <- reactive({
    powerplay_2017_all %>% filter(season == input$yearChoice)
  })
  ...
  bar_chart <- reactive({
    ggplot(iceTime_sub(), aes(x = reorder(team, -prop_goalsperice), y = prop_goalsperice, fill = team)) +
      geom_col() +
      theme_classic() +
      scale_color_viridis_c() +
      labs(x = "Team", y = "Goals Scored Per Minute of Icetime" ) +
  ...
  output$icetimeTable <- renderTable({
    PPFilter <- subset(powerplay_2017_all, season_2017$season == input$yearchoice)
