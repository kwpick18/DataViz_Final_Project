...
icetime_table_data <- powerplay_2017_all %>%
  select(sumgoals, sumice, prop_goalsperice) 
```
...
ui <- fluidPage(
  ...
  tabPanel("IceTime", fluid = TRUE, 
           ...
           sidebarLayout(sidebarPanel(
            ...
            label = "Choose a Team", choices= levels(factor(powerplay_2017_all$team)), selected = "NYR")), 
...
server <- function(input, output, session) {
  ...
  iceTime_sub <- reactive({
    powerplay_2017_all %>% filter(team == input$teamchoice)
  })
  ...
  bar_chart <- reactive({
    ggplot(iceTime_sub(), aes(x = reorder(season, -sumice), y = sumgoals, fill = season)) +
      geom_col() +
      theme_classic() +
...
## For monday 4/18, figure out how to order in descending order in my icetime table so that reader can compare the two visuals easier. 
